...
//        long total = queryFactory.selectFrom(member).fetchCount();
    ...
    public void constant() {
        ...
        System.out.println("========");
        System.out.println(result.get(0).get(Expressions.constant("A")));
    }
    ...
    public void tupleProjection() {
        ...
        //tuple은 리포지토리 안에서만 쓰고 밖으로 내보낼때는 dto로 변환하는걸 추천한다.
        List<Tuple> result = queryFactory
    ...
    private Predicate usernameEq(String usernameParam) {
        if(usernameParam == null){
            return null; //null이면 where안에서 무시됨
        }
    ...
    public void bulkUpdate() {
        ...
        //벌크 연산 주의점, 벌크 연산은 영속성 컨텍스트(1차 캐시)를 무시하고 디비에 바로 날라가버림(영속성 컨텍스트는 변하지 않음)
        ...
        //영속성 컨텍스트가 비어있지않아서 디비를 안감 그래서 비회원으로 이름이 안바뀐게 나옴(repeatable read)
        List<Member> result = queryFactory
                .selectFrom(member)
                .fetch();
